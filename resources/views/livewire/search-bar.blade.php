    ...
    <div class="absolute z-50 mt-4 w-full bg-white/80 rounded-3xl shadow-[0_20px_60px_rgba(0,51,110,0.15)] border border-[#00336E]/5 overflow-hidden backdrop-blur-3xl">
        ...
        @if($results->isNotEmpty())
            <div class="py-4">
                ...
                @foreach($results as $word)
                    <a href="{{ route('word.show', $word->slug) }}" class="flex items-center gap-4 px-8 py-4 hover:bg-brand-accent/10 transition-all group">
                        <div class="flex-shrink-0 w-10 h-10 rounded-full bg-[#00336E]/5 text-[#00336E] flex items-center justify-center font-black text-sm group-hover:bg-brand-accent group-hover:text-[#00336E] transition-all">
        ...
        @else
            <div class="px-8 py-10 text-center">
                ...
                <a href="{{ route('word.create', ['term' => $query]) }}" class="inline-flex items-center justify-center px-8 py-3 bg-[#00336E] text-white text-xs font-black rounded-xl hover:bg-brand-accent hover:text-[#00336E] transition-all uppercase tracking-widest shadow-lg">
                    + Submit "{{ $query }}"
                </a>
            </div>
        @endif
